...
class KittiEvalOdom():
    ...
    def __init__(self, args): #关于评估长度 为了查看闭环的影响  https://github.com/wh200720041/iscloam/issues/11#issuecomment-666942316
        ...
        if args.useSE == '2':
            self.useSE3 = False
            print('[for Oxford]error use only x z and pitch')
        elif args.useSE == '3':
            self.useSE3 = True
            print('[for Oxford]error use all')
    ...
    def calc_sequence_errors(self, poses_gt, poses_result):
        ...
        for first_frame in range(0, len(poses_gt), self.step_size):
            for i in range(self.num_lengths):
                ...
                if not self.useSE3:
                    # 对于oxford radar robot car gt 只是平面运动 评估时只考虑平面 相机系下 x z pitch 即只有绕y轴的旋转
                    pose_delta_gt[0,1] = 0.
                    pose_delta_gt[1,1] = 1.
                    pose_delta_gt[2,1] = 0.
                    pose_delta_gt[1,0] = 0.
                    ...
                    pose_delta_result[0,1] = 0.
                    pose_delta_result[1,1] = 1.
                    pose_delta_result[2,1] = 0.
                    pose_delta_result[1,0] = 0.
                    pose_delta_result[1,2] = 0.
                    pose_delta_result[1,3] = 0.
    ...
    def eval(self, gt_dir, result_dir, 
        ...
        Args:
            ...
            useSE (str): if not None, for oxford data, evaluate use 
                - 2: only consider x z pitch
                - 3: normal way
            seqs (list/None):
                - None: Evalute all available seqs in result_dir
                - list: list of sequence indexs to be evaluated
        """
        seq_list = ["{:01}".format(i) for i in [1, 2] ] #oxford 就一个 1 两个
        ...
        if seqs is None:
            ...
            self.eval_seqs = [int(i[-5:-4]) for i in available_seqs if i[-5:-4] in seq_list] #oxford 1.txt
        else:
            self.eval_seqs = seqs
        ...
        for i in self.eval_seqs:
            self.cur_seq = i
            # Read pose txt
            self.cur_seq = '{:01}'.format(i)
            file_name = '{:01}.txt'.format(i)
            ...
            # for lio-sam 可能出现pose_result的长度多于gt的情况 要截取掉多于gt的那些 #对于oxford 是gt多
